...
class WorkSheetContent:
    ...
    text_of_cells = {
        ...
        "Итого по бригаде с учетом перебазировки": (
            "A26",
            base_font,
            Alignment(wrap_text=True, vertical="center"),
        ),
        "в том числе по категориям рабочих": ("A27", base_font, base_alignment),
        "Вед.инженер-рук. СВ УРЗА СРЗА и АСУТП": (
            "B37",
            Font(size=12, name=Fonts.TNR),
            base_alignment,
        ),
    ...
    def change_cells(cells: dict[str, str], steps: tuple[int, ...]) -> dict[str, str]:
        ...
        for i, cell in enumerate(coordinate_of_cells):
            row, column = cell
            row, column = (
                (row + steps[i], column) if names[i].startswith("r") else (row, column + steps[i])
            )
